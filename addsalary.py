...
def add_fields():
    ...
    Netpay = (salary - deduction) + extratime
    print(salary)
    print(deduction)
    print(extratime)
    print(Netpay)
    ...
    a11 = e11.get()
    insertstmt = (
                "insert into salary(employeeno,name,month,noofdays,noofleaves,overtimeinhours,salary,salaryperday,deduction,extratime,Netpay) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
    ...
    print(
        "EMPLOYEENO:%s\nNAME:%s\nMONTH:%s\nNOOFDAYS:%s\nNOOFLEAVES:%s\nOVERTIMEINHOURS%s\nSALARY:%s\nSALARYPERDAY:%s"
        "\nDEDUCTION:%s\nEXTRATIME:%s\nNETPAY:%s" % (
        e1.get(), e2.get(), e101.get(), e4.get(), e5.get(), e6.get(), e7.get(), e8.get(), e9.get(), e10.get(),
        e11.get()))
