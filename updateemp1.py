...
def update_record():
    ...
    sqlupd = "UPDATE employee SET name='%s',qualification='%s',experience='%s',job='%s',dateofjoin='%s',contact='%s'," \
             "address='%s',email='%s',salary='%s' WHERE employeeno=%d" % (
                 args1, args2, args3, args4, args5, args6, args7,
                 args8, args9, args10)
    a.execute(sqlupd)
